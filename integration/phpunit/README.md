...
Turn copy/paste detection into a **regression test**. Instead of (or in
addition to) running `phpcpd` as a separate CI step, assert that your code
is duplication-free from inside your test suite — so a clone introduced in a
pull request makes the build red, with the offending locations printed in
the failure message.

This is a thin layer over phpcpd-next's **headless mode**
(`Phpcpd::detect()`): it runs detection in-process, with no shelling out to
the binary and no temp files.
...
$this->assertNoDuplication(
    ...
    int $minLines  = 5,
    ?string $algorithm = null,         // null = Rabin-Karp + TokenBag; 'unified' for gapped clones
...
`[inconsistent]` marks a diverged (Type-3) clone — the dangerous kind, where
one copy was patched and its sibling was not.

## Installing it in your project

phpcpd-next ships these classes under the
`LucianoPereira\PhpcpdNext\PHPUnit\` namespace via its **dev autoloader**,
so once `phpcpd-next/phpcpd` is a `require-dev` of your project the trait is
already autoloaded — just `use` it.

> This directory doubles as the canonical example of the integration.
> phpcpd-next's own `tests/SelfDryTest.php` uses this exact trait to
> guarantee its `src/` stays duplication-free — the integration is
